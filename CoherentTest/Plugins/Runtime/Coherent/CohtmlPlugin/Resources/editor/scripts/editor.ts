...
import systemConfigOptions from 'lib/configs/system_config';
...
export namespace coui {
    ...
    export class Editor {
        ...
        cleanupHtmlExport(htmlContents: string): string {
            ...
            cleanedHtml = cleanedHtml.replace(/(background-image|src|mask-image|-webkit-mask): url\((.*?)\)/gi, (RegSelection, regUrl, RegGroup) => {
                const quotes = RegGroup[0] === '"' ? `"` : `'`;
                const imageURL = this.handleRelativePath(RegGroup);

                return `${regUrl}: url(${quotes}${imageURL}${quotes})`;
