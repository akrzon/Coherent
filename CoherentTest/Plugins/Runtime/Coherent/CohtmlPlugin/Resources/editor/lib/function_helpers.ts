...
export default {
    ...
    getElementVertices(widget: IWidget): IWidgetGeometry {
        ...
        // If the element is not nested, the scene border must be subtracted
        if (element.parentElement.id === 'scene') {
            sceneBorder = runtimeEditor.sceneBorderSize;
        }
        ...
        let top = this.calculatePosition(element, 'top', sceneBorder);
        let left = this.calculatePosition(element, 'left', sceneBorder);
    ...
    calculatePosition(element: HTMLElement, style: string, sceneBorder: number): string {
        const offset = element.style.position === 'relative' ? this.offset(element)[style] : 0;
        const result = this.getComputedStyleNumber(element, style) + this.getComputedStyleNumber(element.parentElement, `border-${style}-width`) - sceneBorder + offset;
        return result + 'px';
    },

    getComputedStyleNumber(element: Element, style: string, parse: boolean = true): string | number {
        let compStyle = window.getComputedStyle(element, null).getPropertyValue(style);
        // elements with position relative or with explicitly set properties to auto
        // need to use "0" instead of "auto"
        compStyle = compStyle === 'auto' ? '0' : compStyle;

        return parse ? parseInt(compStyle) : compStyle;
    },
     ...
     * */
    buildStringProps(values: Array<string | number>, sort?: boolean): string {
    ...
    translateDataBindNames: function (value, toScene) {
        if (toScene) {
            ...
            } else if (value === 'bindStyleBackgroundImageUrl') {
                translatedNames = translatedNames.replace('bind-style-background-image-url', 'background-image');
            }
            ...
            if (value === 'class-toggle') {
                ...
                newValue = 'bindStyle' + newValue + 'Url';
            }
    ...
    openPathHandler(content: string, path: string): string {
        ...
        let cleanedContent = splitContentByJsScript[0]
            ...
            .replace(/(background-image|src|mask-image|-webkit-mask): url\((.*?)\)/gi, (regSelect, regUrl, regGroup) => {
                ...
                if (extension === Enums.extensions.ttf || extension === Enums.extensions.ttf) {
                    return `${regUrl}: ${this.pathChanger({regGroup: url, path, isFont: true})}`;

                // responsive images
                } else {
                    return `${regUrl}: ${this.pathChanger({regGroup: url, path, isResponsive: true})}`;
                }
            })
