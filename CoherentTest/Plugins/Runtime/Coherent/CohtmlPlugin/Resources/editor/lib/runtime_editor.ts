...
export default class RuntimeEditor {
    ...
    _assetChosenHandler() {
        ...
        engine.on('AssetChosen', function (url, id, type) {
            ...
            if (url !== '' || type === 'image' || type === 'video') {
                self._sceneActionState.primaryAction = 'new action';
                if (self.sceneAssetSelected) {
                    self.sceneAssetChosen(url, type);
                } else {
                    self.assetChosen(url, id, type);
                    self.sceneAssetChosen(url, type);
                }
            }

            // reset actions
            self._sceneActionState.initialCreationOfWidgetByExternalFile = false;
            self._sceneActionState.insertMaskImage = false;
        });
    }
    ...
    applyUploadedImage($element, id, url, type) {
        ...
        if (!self._sceneActionState.insertMaskImage) {
            ...
            _tempImg.onload = function () {
                ...
                if (firstUpload) {
                    ...
                    if (widthUnit !== 'auto') {
                        ...
                        width = unitsConvertor.convertPixelToVw(pixelWidth) + 'vw';
                        self._setProperties(widget, null, 'units', 'width', 'vw');
                    }
                    ...
                    if (heightUnit !== 'auto') {
                        ...
                        height = unitsConvertor.convertPixelToVh(pixelHeight) + 'vh';
                        self._setProperties(widget, null, 'units', 'height', 'vh');
                    }
                }

                self._setGeometry($element, widget, id, 'geometry',
                    'width', width);
                self._setGeometry($element, widget, id, 'geometry',
                    'height', height);

                self.clearSelectedElements();
                self.selectJstreeItem(id);
    ...
    insertVideo(id, url) {
        ...
        var widget = self.mappedWidgets[id].widget;
        ...
        $element.on('loadedmetadata', function () {
            ...
            self.saveProperties(id, 'css property', 'url',
                url);

            // skip geometry manipulation record in undo stack
            self._skipUndoRedoSteps = 2;
            self._setGeometry($element, widget, id, 'geometry',
                'width', width);
            self._setGeometry($element, widget, id, 'geometry',
                'height', height);
    ...
    _attachLibraryClickEvent() {
        ...
        $(document).on('click', '.convert-widget', function () {
            ...
            System.import(editDataUrl + '?' + date.getTime() + '!text').then(function (data) {
                ...
                let newScene = couiEditor.handleFileContent(data, componentName, self.editorTabName, true, false);
        ...
        $(document).on('click', '#assets-bar .fa-remove', function (event) {
            ...
            const filteredAssets = self._filterAssets(couiEditor.assets[deleteType], url);
            ...
            if (deleteType === 'widget') {
                ...
                confirmDelete.create(Enums.warnMessages.deleteComponents(name)).then(function (result) {
                    if (result === 'yes') {
                        self.removeWidgetAsset(name, 'widget');
                        self.assetsDOM = createVDOM(self.createAssetDOM(couiEditor.assets).template);
                    }
                });
            } else if (deleteType === 'font') {
                const fontName = url.replace(/\.[^/.]+$/, '').replace(/^.*[\\\/]/, '');
                const matchedElements = self._getElementsWithFont(self.mappedWidgets, fontName);

                if (!isActive && !matchedElements.length) {
                    removeAsset(vListIndex);
                } else {
                    const warning = self._fontFamilyWarning(fontName, matchedElements);
                    ...
                    vex.dialog.open({
                        ...
                        callback(value) {
                            // on confirm
                            if (value) {
                                self._resetFontFamily(matchedElements);
                                removeAsset(vListIndex);
                            } else {
                                return;
                            }
                ...
                if (deleteType !== 'style' &&
                    ...
                    if (self[sceneWidgetSelectType].attr('src') === url) {
                        self[sceneWidgetSelectType].fadeOut(100, function () {
                            self[sceneWidgetSelectType].attr('src', '');
                            self[sceneWidgetSelectType].css('display', 'none');

                            self.sceneAssetPreviewStandin.css('display', 'block');
                            self.sceneAssetPreviewStandin.fadeIn(100);
                        });
                    }
                } else {
                    // converting style to styles or script to scripts to refer to the scene //
                    url = url.replace('uiresources/', '');
                    self._toScene(url, deleteType, false);
                }
            ...
            function removeAsset(index) {
                ...
                self.virtualList.removeAsset(index);
                self.assetsDOM = createVDOM(self.createAssetDOM(couiEditor.assets).template);
            }
        ...
        $(document).on('click', '.script-or-style', function (event) {
            ...
            if ($this.hasClass('fa-circle-o')) {
                ...
                self._toScene(dataURL, dataType, true);
            } else {
                ...
                self._toScene(dataURL, dataType, false);
            }
            couiEditor.tabEdited(true);
            event.stopImmediatePropagation();
        });
        ...
        $(document).on('click', '.asset-file-name', function (event) {
            ...
            self._switchPreviewWindow(url, type);
        ...
        $('#assets-bar').kendoDraggable({
            ...
            hint(element) {
                ...
                self.draggedElement = $(newElement[0]);
                self.draggedElement.css('z-index', '999999');
                return newElement;
            },
            dragstart(event) {

                var scale = self.tab.sceneTransformMatrix[0];

                var elementLeft = parseFloat(self.draggedElement.css('left'));
                var elementTop = parseFloat(self.draggedElement.css('top'));
        ...
        self.sceneWrapper.kendoDropTarget({
            group: 'assetsGroup',
            dragenter(e) {
            ...
            drop(e) {
                ...
                switch (draggedElement.tagName) {
                    case 'IMG':
                        ...
                        break;
                    case 'DIV':
                        couiEditor.openFiles[self.editorTabName].components._insertToTheScene(draggedElement, e);
                        return;
                    case 'VIDEO':
                        type = 'video';
                        break;
                ...
                const currentWidget = self._prepareObjEl(type);
                const elementId = currentWidget.id;
                const offsets = self._getSceneOffset(e);
                ...
                self.createElementOnTheScene(elementId, type, currentWidget);

                self.applyProperties(elementId, 'css property', 'top', offsets.y);
                self.applyProperties(elementId, 'css property', 'left', offsets.x);
                self.applyProperties(elementId, 'css property', 'width', draggedElement.width);
                self.applyProperties(elementId, 'css property', 'height', draggedElement.height);

                self.highlightSelectedEl($(document.getElementById(elementId)));
                ...
                self._sceneActionState.elementWasInserted = true;

                type = type === 'responsiveImage' ? 'image' : 'video';

                const url = draggedElement.getAttribute('src');
                self.assetChosen(url, elementId, type);
            }
        });
    }
    ...
    _switchPreviewWindow(url, type) {
        if (type !== 'style' && type !== 'script' && type !== 'font') {
            ...
            var sameType = selectedElement[0].tagName === self[sceneWidgetSelectType][0].tagName;

            $(selectedElement).fadeOut(50, function () {
                if (!sameType) {
                    selectedElement.attr('src', '');
                    self[sceneWidgetSelectType].attr('src', url);

                    selectedElement.css('display', 'none');
                    selectedElement.css('opacity', '1');

                    self[sceneWidgetSelectType].css('display', 'block');
                } else {
                    self[sceneWidgetSelectType].attr('src', url);
                }
                self[sceneWidgetSelectType].fadeIn(50);
            });
        }
    }
    ...
    _addWidgetHandler() {
        ...
        $('.widget-name').on('click', function () {
            ...
            var currentWidget = self._prepareObjEl(elementType);
            var elementId = currentWidget.id;

            self._sceneActionState.elementWasInserted = false;

            if (elementType === 'image' ||
                elementType === 'responsiveImage' ||
                elementType === 'video') {
                self.interactElementsState = 'resize';
                self._sceneClickingState.draggingCreation = false;
                self._sceneActionState.initialCreationOfWidgetByExternalFile = true;
                self.createElementOnTheScene(elementId, elementType,
                    currentWidget);
                ...
                self._sceneClickingState.draggingCreation = true;
                self.initElementOnScene(elementId, elementType,
                    currentWidget);
            }
    ...
    initElementOnScene(elementId,
        ...
        function initWidget(event) {
            ...
            if (trigger === 'scene' ||
                ...
                self.mappedWidgets[trigger] !== undefined) {

                self.scene.widgets.push(currentWidget);
                ...
                if (isParentCenterPane ||
                    ...
                    var sceneOffset = $(self.scenePreviewEl).offset();
                    var scale = self.tab.sceneTransformMatrix[0];

                    var x = (event.pageX - sceneOffset.left) / scale;
                    var y = (event.pageY - sceneOffset.top) / scale;

                    self.createElementSimple(currentWidget);

                    // Map objects
                    self.mappedWidgets[elementId] = {
                        widget: currentWidget
                    };

                    self.createElementEvent = event;

                    let $widgetEl: any = $('#' + elementId);

                    self.applyProperties(elementId, 'css property',
                        'top', y);
                    self.applyProperties(elementId, 'css property',
                        'left', x);

                    self.highlightSelectedEl($widgetEl);

                    x = unitsConvertor.convertPixelToVw(x);
                    y = unitsConvertor.convertPixelToVh(y);

                    currentWidget.geometry.top = y.toFixed(self.formatTo + 1) + 'vh';
                    currentWidget.geometry.left = x.toFixed(self.formatTo + 1) + 'vw';

                    isOnStage = true;
                }
            }
            self._sceneClickingState.draggingCreation = false;
            self.WidgetSelection.attachHandlers();
        ...
        function applyVisualWidget() {
            ...
            if (isOnStage) {
                self.interactElementsState = 'resize';
                self.jsTreeAddNode(elementType, elementId, '#', 'last', true);

                var isWidget = self.tab.tabWidgetState.editWidget;
                if (isWidget) {
                    self.placeWidgetOnScene(elementId);
                }

                // timeline creation
                self.Animations.createAnimationWidget(currentWidget);

                self.saveUndoRedoAfterCreateElement(elementId, currentWidget);
            }
            self._sceneClickingState.draggingCreation = false;
    ...
    _removeFilesHandler() {
        ...
        $('.remove-files').on('click', function () {
            ...
            self._sceneActionState.primaryAction = 'new action';
            self.deleteFile(index, type, originLink);
        });
    }
    ...
    setAspectRatio(type) {
        ...
        switch (type) {
            case 'aspectRatio5_4':
                self.scene.sceneSize.width = '1000px';
                self.scene.sceneSize.height = '800px';
                break;
            case 'aspectRatio4_3':
                self.scene.sceneSize.width = '800px';
                self.scene.sceneSize.height = '600px';
                break;
            case 'aspectRatio16_10':
                self.scene.sceneSize.width = '1440px';
                self.scene.sceneSize.height = '900px';
                break;
            case 'aspectRatio16_9':
                self.scene.sceneSize.width = '1600px';
                self.scene.sceneSize.height = '900px';
                break;
            case 'aspectRatio16_9_full_hd':
                self.scene.sceneSize.width = '1920px';
                self.scene.sceneSize.height = '1080px';
                break;
            case 'aspectRatio_custom':
                self.scene.sceneSize.width = sceneSize.width;
                self.scene.sceneSize.height = sceneSize.height;
                break;
        }

        self.scenePreviewEl.style.width = sceneSize.width;
        self.scenePreviewEl.style.height = sceneSize.height;
    ...
    highlightSelectedEl(element, elementsIds?) {
        ...
        self.WidgetSelection.setSelectedState(true);
        ...
        if (!self.iframe) {
            if (elementsIds) {
                for (var i = 0; i < elementsIds.length; i++) {
                    ...
                    self.appendManipulationMasks(currState, isLocked, elem);
                }
            } else {
                isLocked = (element.attr('data-lock') === 'true');
                self.appendManipulationMasks(currState, isLocked, element);
            }
        }
    }
     ...
     */
    scaleManipulationMasks($element: JQuery | null, sceneScale: number) {
        if (sceneScale >= 1) {
            ...
            if (this.getSelectedWidgetId() && !this._sceneClickingState.draggingCreation && $element === null) {
    ...
    changeSceneScale(sceneScale) {
        ...
        this.scaleManipulationMasks(null, 1 / sceneScale);
    }

    /**
     * Append manipulation mask over selected elements
     * @function appendManipulationMasks
     * @memberOf module:lib/runtime_editor.RuntimeEditor
     * @param {string} currState - resize or rotate element
     * @param {boolean} isLocked
     * @param {JQuery} $element - jquery element
     */
    appendManipulationMasks(currState: string, isLocked: boolean, $element: JQuery) {
        ...
        if (currState === 'resize-corners') {
            self._setRightClickHandling(true);
        } else {
            self._setRightClickHandling(false);
        }
        ...
        this.scaleManipulationMasks($element, 1 / scale);
        ...
        function clickSelect(e) {
            ...
            if (!self._dragFlag && !self.isInPanningMode) {
                self.blurOutTextInputs();
                self._elementSelectionHelper($element[0].id);

                if (self.currentElementsSelection.length === 1) {
                    if (self.propertiesElBar.is(':empty')) {
                        self.selectJstreeItem($element[0].id);
                    }
                }

                if (self.interactElementsState === 'resize') {
                    self.interactElementsState = 'rotate';
                } else if (self.interactElementsState === 'rotate') {
                    self.interactElementsState = 'resize';
                }
                self.highlightSelectedEl($element);
            }
        ...
        if (!isLocked && !self.isInPanningMode) {
    ...
    _deleteWidgetsAndAnimations(id) {
        ...
        function deleteWiget(widgets) {
            ...
            for (var i = 0; i < len; i++) {
                var widget = widgets[i];
                self._lastDeletedAnimationWidgets[widget.id] = $.extend(true, {}, self.scene.animations[widget.id]);
                self.Animations.deleteWidget(widget.id);
                delete self.mappedWidgets[widget.id];
                if (widget.children.length > 0) {
    ...
    saveUndoRedo(widgetId, propGroup, prop, value, originalPropGroup) {
        ...
        function createUndoRedo(type) {
            ...
            self.createUndoRedoCommand(type, widgetId, undoGroup, propGroup, prop, value);
        }
    }
    ...
    createUndoRedoCommand(sceneActionType, widgetId, originalPropGroup, propGroup, prop, value: any) {
        ...
        switch (originalPropGroup) {
            case 'geometry':
                action[type].value = self.mappedWidgets[widgetId].widget[propGroup][prop] || '0';
                break;
            case 'image':
            ...
            case 'units':
                if (prop === 'fontSize') {
                    action[type].value = self.mappedWidgets[widgetId].widget[prop] || 'auto';
                } else if (helpers.isTransformProperty(prop)) {
                    action[type].value = self.mappedWidgets[widgetId].widget[propGroup][prop] || Enums.TransformDefaultValue[prop];
                } else if (self.mappedWidgets[widgetId].widget[propGroup] !== undefined) {
                    action[type].value = self.mappedWidgets[widgetId].widget[propGroup][prop] || 'auto';
                } else {
                    action[type].value = 'auto';
                }
                break;
            case 'css property':
                action[type].value = self.mappedWidgets[widgetId]
                    .widget[prop];
                break;
            case 'element-text':
            case 'element-class':
            ...
            case 'dataBindings':
                ...
                action[type].value = self.mappedWidgets[widgetId].widget[propGroup][prop] || '';
                ...
                if (helpers.isTransformProperty(prop)) {
                    if (type !== 'deleteKeyframe') {
                        ...
                        if (prop === 'perspective-origin-x' || prop === 'perspective-origin-y') {
                            widgetValue = self.mappedWidgets[widgetId].widget[originalPropGroup][prop] || Enums.TransformDefaultValue[prop];
                        } else if (prop !== 'transform-origin') {
                            widgetValue = self.mappedWidgets[widgetId].widget[originalPropGroup][prop] || Enums.TransformDefaultValue[prop];
                        } else {
                            var transforms = self.mappedWidgets[widgetId].widget[originalPropGroup];
    ...
    initBoxShadowColorPicker(widget) {
        ...
        var setBoxShadowColor = function (color) {
            self.saveProperties(widget.id, 'boxShadow', 'color', color);

            var boxShadowProps = self.buildBoxShadowProperty(widget);
            self.applyProperties(widget.id, 'css property', 'boxShadow', boxShadowProps);
        };
        ...
        $boxShadowColorPicker.kendoColorPicker({
            ...
            open: function () {
                ...
                self.colorPickerLeftReposition();
            },
            close: function () {
                // TODO: Ugly - info on first comment
                self.colorPickerRemoveLeftReposition();
    ...
    initFilterColorPicker(widget) {
        ...
        var setFilterColor = function () {
            ...
            self.saveProperties(widget.id, '-webkit-filter', 'dropShadowColor', color);
            helpers.splitCssStringProperties(widget.id, el.style['webkitFilter'], 'webkitFilter', 'dropShadowColor', color, function (val) {
                self.applyProperties(widget.id, 'css property', '-webkit-filter', val);
            });
        };
    ...
    _setUnits($this, widget, currentWidgetId, propGroup, propKey, value) {
        ...
        // update nested element's width and height properly when the unit is set to %
        if ((propKey === 'width' || propKey === 'height') && helpers.getUnitStyle(newUnitValue) === '%') {
    ...
    _setBackgroundColor($this, widget, id, group, key, value) {
    ...
    _setBorderColor($this, widget, id, group, key, value) {
        this.setPropertiesBarValue(id, group, key, value);
        this.saveProperties(id, 'css property', 'borderColor', value);
        this.applyProperties(id, 'css property', 'borderColor', value);
    }

    _setColor($this, widget, id, group, key, value) {
        this.setPropertiesBarValue(id, group, key, value);
        this.saveProperties(id, 'css property', 'color', value);
        this.applyProperties(id, 'css property', 'color', value);
    }

    _setPositionType(widget: IWidget, value: PropertyPosition) {
        const $kendoSelectTop = $('select[data-property-set="units"][data-property-key="top"]').data('kendoDropDownList');
        const $kendoSelectLeft = $('select[data-property-set="units"][data-property-key="left"]').data('kendoDropDownList');

        if (value === 'relative') {
            $('input[data-property-key="left"]').prop('disabled', true);
            $('input[data-property-key="top"]').prop('disabled', true);

            this.setPropertiesBarValue(widget.id, 'geometry', 'left', 'auto');
            this.setPropertiesBarValue(widget.id, 'geometry', 'top', 'auto');
            ...
            if ($kendoSelectTop) {
                ...
                const widgetCoordinates = element.getBoundingClientRect();
                const sceneCoordinates = document.getElementById('scene').getBoundingClientRect();

                $kendoSelectTop.enable(true);
                $kendoSelectLeft.enable(true);

                this._undoCreationStepsLength += 2;
                let top = ((widgetCoordinates.top - sceneCoordinates.top) / this.getSceneScale()) - this.sceneBorderSize;
                let left = ((widgetCoordinates.left - sceneCoordinates.left) / this.getSceneScale()) - this.sceneBorderSize;

                // subtract element's parent offsets to get the proper current widget top and left
                const parentsOffset = helpers.getAbsolutePosition(element, 'scene');
                top -= parentsOffset.top;
                left -= parentsOffset.left;

                this._setProperties(widget, null, 'geometry', 'left', `${unitsConvertor.convertPixelToVw(left).toFixed(2)}vw`);
                this._setProperties(widget, null, 'geometry', 'top', `${unitsConvertor.convertPixelToVw(top).toFixed(2)}vw`);
            }
        }

        this.applyProperties(widget.id, 'css property', 'position', value);
    }

    _setGeometry($this, currentWidget, currentWidgetId, propGroup, propKey, value) {
        let newVal;
        let oldVal;
        let originalPropGroup = propGroup;
        let saveType = 'css property';
        let units;
        let valueAsNumber;
        ...
        if (value === undefined) {
            if (propKey === 'position') {
                ...
                if (propKey === 'rotation') {
                    const rad = unitsConvertor.toRadians(valueAsNumber);
                    newVal = rad + 'rad';
    ...
    _setEventsByDropDown($this,
        ...
        if (selectedDropdown === Events.Engine.blueprint) {
            ...
            self.createModalJsEditor(propKey,
    ...
    _setFilter($this, currentWidget, currentWidgetId, propGroup, propKey, value) {
        ...
        helpers.splitCssStringProperties(currentWidgetId, el[0].style[propGroup], 'webkitFilter', propKey, sliderVal, function (val) {
            if (propKey !== 'dropShadowBlur' && propKey !== 'dropShadowColor' &&
                propKey !== 'dropShadowX' && propKey !== 'dropShadowY') {
                self.saveProperties(currentWidgetId, saveType, propKey, sliderVal, originalPropGroup);
            }
            self.applyProperties(currentWidgetId, 'css property', '-webkit-filter', val);
        });
    }
    ...
    editAnimationClassName(widgetId, oldClassName, newClassName) {
        if (newClassName.trim() === '') {
            ...
            this._showValidationWarning(Enums.warnMessages.animationNameNotAllowed, () => {
                let oldName = self.mappedWidgets[widgetId].widget.className;
    ...
    initPropertiesBarHandlers(widget, propertiesBarId) {
        ...
        $propertyInputs.on('focusout', function (event) {
            ...
            self._sceneActionState.primaryAction = 'new action';
            self._setProperties(widget, $(this));
        });
        ...
        $propertiesType.find('.add-js-code').on('click', function () {
            ...
            const val = self.mappedWidgets[widget.id].widget.events[propKey].javascriptFunction;
            const Events = Enums.SavePropertiesTypes.Events;

            self.createModalJsEditor(propKey, Events.Local.javascript, val, propGroup);
        });
        ...
        function action(e: KeyboardEvent) {
            ...
            if (!self.inputValidator.validate(value, propKey)) {
                this.value = oldValue;
                return;
            }
    ...
    _setProperties(widget, $this, group?, key?, value?) {
        ...
        if ($this) {
            ...
            if (shouldValidate) {
                ...
                if (!self.inputValidator.validate(_value, propKey)) {
                    $this.val(oldValue);
                    return;
                }
            }
        ...
        if (propGroup !== undefined) {
            var currentWidget = self.mappedWidgets[widget.id].widget;
            ...
            if (propGroup === 'geometry') {
                self._setGeometry($this, currentWidget, currentWidgetId, propGroup, propKey, value);
                self.computeSelectCorners($('#' + currentWidgetId));
            }

            if (propGroup === 'image') {
                self.insertImage(currentWidgetId, value);
            }

            if (propGroup === 'transform') {
                self._setTransform($this, currentWidget, currentWidgetId, propGroup, propKey, value);
                self.computeSelectCorners($('#' + currentWidgetId));
            }

            if (propGroup === 'perspective-origin') {
                self._setPerspectiveOrigin($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'transform-origin') {
                self._setTransformOrigin($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === '-webkit-filter') {
                self._setFilter($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'boxShadow') {
                self._setBoxShadow($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'events') {
                self._setEvents($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'dataBindings') {
                self._setBindings($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }
            ...
            if (propGroup === 'font') {
                self._setFonts($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'element-id') {
                self._setId($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'element-text') {
                self._setText($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'element-class') {
                self.setClassName($this, currentWidget.id, value, true);
            }

            if (propGroup === 'checked') {
                self._setChecked($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }

            if (propGroup === 'attrs') {
                self._setAttrs($this, currentWidget, currentWidgetId, propGroup, propKey, value);
            }
            ...
            if (propGroup === 'styles') {
                ...
                self._setStyles($this, currentWidget, currentWidgetId, propGroup, propKey, value);
    ...
    backgroundPropertiesBarHandler(widget) {
        ...
        handlebarObj.env = couiEditor.preferences.couiEnvironment;
        var htmlProperties =
            self.templateBackgroundProperties(handlebarObj);
        ...
        $backgroundColorPicker.kendoColorPicker({
            ...
            open: function () {
                ...
                $backgroundColorPicker.on('mouseup', function () {
                    ...
                    self._sceneActionState.primaryAction = 'new action';
                    self.saveProperties(widget.id, 'css property', 'background', background);
                });
            },
            close: function () {
                // TODO: Ugly
            ...
            select: function (event) {
                ...
                self.applyProperties(widget.id, 'css property', 'background', event.value);
                self.saveProperties(widget.id, 'css property', 'background', event.value);
            },
            value: self.isColorbackground(widget.background),
            opacity: true
        });
    ...
    _renderTextPropertiesPanel(widget) {
        ...
        $textColorPicker.kendoColorPicker({
            ...
            open: function () {
                ...
                $('body').on('mouseup', function () {
                    ...
                    self.saveProperties(widget.id, 'css property', 'color', color);
                });
            },
            close: function () {
                // TODO: Ugly - info on first comment
            ...
            select: function (event) {
                ...
                self.applyProperties(widget.id, 'css property', 'color', event.value);
                self.saveProperties(widget.id, 'css property', 'color', event.value);
            },
            value: widget.color,
            opacity: true
        });
        ...
        const fontStyles = self.scene.fonts.map(font => {
        ...
        $('.font-family').kendoDropDownList({
            ...
            change() {
                self._sceneActionState.primaryAction = 'new action';
                self._setProperties(widget, null, 'styles', 'fontFamily', this.value());
            },
            ...
            dataBound() {
                ...
                if (helpers.fontIsPresent(widget.styles.fontFamily, self.scene.fonts)) {
                ...
                $borderColorPicker.kendoColorPicker({
                    ...
                    open: function () {
                        ...
                        $('body').on('mouseup', function () {
                            ...
                            self.saveProperties(widget.id, 'styles', 'borderColor', color);
                        });
                    },
                    close: function () {
                        // TODO: Ugly - info on first comment
                    ...
                    select: function (event) {
                        ...
                        self.applyProperties(widget.id, 'css property', 'borderColor', event.value);
                        self.saveProperties(widget.id, 'styles', 'borderColor', event.value);
                    },
    ...
    filtersPropertiesBarHandler(widget) {
        ...
        $('.dismiss-filter-drop-shadow').on('click', function () {
            self._undoCreationStepsLength = 4;
            self._setFilter(null, widget, widget.id, '-webkit-filter', 'dropShadowX', '0px');
            self._setFilter(null, widget, widget.id, '-webkit-filter', 'dropShadowY', '0px');
            self._setFilter(null, widget, widget.id, '-webkit-filter', 'dropShadowBlur', '0px');
            self._setFilter(null, widget, widget.id, '-webkit-filter', 'dropShadowColor', 'rgb(0, 0, 0)');
    ...
    _initTransformHandlers(widget) {
        ...
        $('.remove-property').on('click', function (event) {
            if (widget.transform) {
                ...
                self.selectKendoDropdownItem('units', key, units);
                self.setUnitNumberToolbar(key, defaultValue);

                self._setTransform(null, widget, widget.id, 'transform', key, defaultValue);
                self.computeSelectCorners($(`#${widget.id}`));
        ...
        $('.dismiss-transform').on('click', function (event) {
            if (widget.transform) {
                ...
                self._undoCreationStepsLength = keys.length;
                for (var i = 0; i < keys.length; i++) {
                    var defaultValue = Enums.TransformDefaultValue[keys[i]];
                    var units = helpers.getUnitStyle(defaultValue);

                    self.selectKendoDropdownItem('units', keys[i], units);
                    self.setUnitNumberToolbar(keys[i], defaultValue);

                    self._sceneActionState.primaryAction = 'new action';
                    self._setTransform(null, widget, widget.id, 'transform', keys[i], defaultValue);
                    self.computeSelectCorners($(`#${widget.id}`));
        ...
        $('.dismiss-transform-origin').on('click', function () {
            if (widget.transform) {
                ...
                self._undoCreationStepsLength = 2;
                for (var i = 0; i < keys.length; i++) {
                    var defaultValue = Enums.TransformDefaultValue[keys[i]];
                    var units = helpers.getUnitStyle(defaultValue);
                    ...
                    self._sceneActionState.primaryAction = 'new action';
                    self._setTransformOrigin(null, widget, widget.id, 'transform-origin', keys[i], defaultValue);
                    self.computeSelectCorners($(`#${widget.id}`));
                    delete widget.transform[keys[i]];
                }
                self.exportScene();
        ...
        $('.dismiss-perspective-origin').on('click', function () {
            if (widget.transform) {
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    self._sceneActionState.primaryAction = 'new action';
                    self._setPerspectiveOrigin(null, widget, widget.id, 'perspective-origin', keys[i], defaultValue);
                    self.computeSelectCorners($(`#${widget.id}`));
                    delete widget.transform[keys[i]];
                }
                self.exportScene();
            ...
            callback: function (data) {
                if (data) {
                    ...
                    if (propGroup === 'sceneEvents') {
                        ...
                        self.scene.sceneEvents[propKey] = val;
                    } else {
                        var currentWidgetId = $('#selected-element')
                            .attr('data-element-id');

                        self.saveProperties(currentWidgetId, propGroup, propKey, val, originalPropGroup);
                    }
    ...
    mapLoadedScene(widgets) {
        ...
        widgets.map(function (widget) {
            ...
            self.mappedWidgets[widget.id] = {
                widget: widget
            };

            if (sceneLen > 0) {
                return self.mapLoadedScene(widget.children);
            }
        });
    ...
    buildJsonInitJstree(widgets) {
        ...
        function jsonTreeBuild(widget) {
            ...
            node.text = self.jsTreeExtraItemHtml(type, widget.id);
    ...
    moveWidgetScenePreview($currentEl, parentElNew, position, oldParentPos) {
        ...
        if (position > oldParentPos) {
            parentElNew.children().eq(position).after($currentEl);
        }
    ...
    _sceneJsTree(data) {
        var self = this;
        var inWidgetEditor = couiEditor.openFiles[self.editorTabName].tab.tabWidgetState.editWidget;
        $('#sceneTree').jstree({
            core: {
                rules: {
                    creatable: true
                ...
                check_callback: function (operation, node, node_parent, node_position, more) {
                    if (node_parent.id !== '#') {
                        ...
                        const preventNesting = self.mappedWidgets[widget].widget.type === 'widget';

                        if (preventNesting && !couiEditor.openFiles[self.editorTabName].tab.tabWidgetState.editWidget) {
                            return false;
                        }
                    }
                }
            ...
            self.widgetVisualSceneState(allNodes);
            self.jstreeDNDHandler();

            if (inWidgetEditor) {
                jsTree.open_all();
            }
            self.rightPanel.scrollTop(self.tab.scrollIndex.rightPanel);
            ...
            let $parentElNew = $(self.scenePreviewEl);
            ...
            self.selectJstreeItem(currentNodeId);
            self.moveWidget(currentNodeId, oldParentDataId, newParentDataId, position, oldParentPos);
            self.moveWidgetScenePreview($currentEl, $parentElNew, position, oldParentPos);

            $jsTree.jstree().open_node(currentParentId);

            self.highlightSelectedEl($currentEl);
            self._childElMoveCheck(currentNodeId, newParentDataId, oldParentDataId);

            // undo/ redo
            var actionState = self.getRedoUndoPrimaryState();
            ...
            if (self._sceneActionState.primaryAction === 'new action') {
                self._undoCreationStepsLength = 3;
                self.repositionElementAfterReorder($currentEl[0], $parentElNew[0], elementAbsolutePosition);
            }

            self._sceneActionState.moveElement = true;
            self.createUndoRedoCommand(actionState, currentNodeId, null,
                null,
                null, moveInfo);
            ...
            self.Timeline.unhighlightAllTracks();
            self.Timeline.unselectAllKeyfames();

            var widgetId = data.node['li_attr']['data-id'];
            var currenetRenderedPropertiesId = $('#selected-element').attr('data-element-id');
            if (currenetRenderedPropertiesId !== widgetId) {
                // set current selection
                self.currentElementsSelection[0] = widgetId;
                self.currentParentElementsSelection[0] = widgetId;
                let $element: any = $('#' + widgetId);
                self.interactElementsState = 'resize';
                self.highlightSelectedEl($element);
                self.Timeline.toggleElementTimeline($element, true);

                // check creation method of the element
                // if is by dragging create properties bar after drag end
                if (!self._sceneClickingState.draggingCreation) {
                    self.renderProperties(self.mappedWidgets[widgetId].widget);
                }
            }
            ...
            self.widgetVisualSceneState(data.node);
    ...
    scenePanning() {
        ...
        this.sceneWrapper.panzoom('option', {
            ...
            disablePan: !self.isInPanningMode
        });
    }
